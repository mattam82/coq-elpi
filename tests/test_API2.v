...
Elpi Query lp:{{
  ...
  coq.locate "P.T" GR,
  coq.env.add-const "id" (fun `a` (pglobal GR UI) a\ a) _ _ _,
...
Elpi Query lp:{{
  ...
  coq.locate "P.T" GR,
  coq.env.add-axiom "idT" (prod _ (pglobal GR _) _\ (pglobal GR _)) _,
  coq.env.end-module-type _.
