...
let not_univ_poly = NotUniversePolymorphic

type poly_flags = bool (* Just poly *)
let default_poly_flags = false
let univ_poly p = p
...
let univ_poly p = PolyFlags.univ_poly p
let default_poly_flags = PolyFlags.default

type universe_decl_option = UState.universe_decl option
let not_univ_poly = None
let univdecl_of_tuple x sx y sy : UState.universe_decl =
  let open UState in
  let univs, variances = List.split x in
  let variances = if List.for_all Option.is_empty variances then None else Some variances in
  { univdecl_qualities = [];
    univdecl_extensible_qualities = true;
    univdecl_elim_constraints = Sorts.ElimConstraints.empty;
    univdecl_instance = univs;
    univdecl_extensible_instance = sx; (* Can new universes be added *)
    univdecl_variances = variances; (* Universe variance information *)
    univdecl_univ_constraints = univ_csts_of_list y; (* Declared univ constraints *)
    univdecl_extensible_constraints = sy; (* Can new constraints (elim or univ) be added *) }

let univdecl_to_tuple udecl =
  let open UState in
  let inst = match udecl.univdecl_variances with
    | None -> List.map (fun x -> x, None) udecl.univdecl_instance
    | Some vs -> List.map2 (fun x y -> x, y) udecl.univdecl_instance vs
  in
  inst, udecl.univdecl_extensible_instance, univ_csts_to_list udecl.univdecl_univ_constraints, udecl.univdecl_extensible_constraints
...
let default_options () = {
  ...
  poly = default_poly_flags;
  universe_decl = not_univ_poly;
  reversible = None;
  keepunivs = None;
  redflags = None;
  no_tc = None;
...
let make_options ~hoas_holes ~local ~warn ~depr ~primitive ~failsafe ~ppwidth
  ~pp ~pplevel ~using ~inline ~uinstance ~poly ~universe_decl ~reversible ~keepunivs
  ...
  { hoas_holes; local; user_warns; primitive; failsafe; ppwidth; pp;
    pplevel; using; inline; uinstance; poly; universe_decl; reversible; keepunivs;
    redflags; no_tc; algunivs; }
...
[%%if coq = "9.0" || coq = "9.1" || coq = "9.2"]
let get_poly_option state map = false
...
let get_options ~depth hyps state =
  ...
  let get_redflags_option () =
    ...
    let poly = get_poly_option state map in
...
let info_of_undefined_evar ~env ~sigma ~section k =
...
let info_of_defined_evar ~env ~sigma ~section k =
  let open Context.Named in
  let evi = Evd.find_defined sigma k in
  assert(Option.has_some evi);
  let info = Evarutil.nf_evar_info sigma (Option.get evi) in
  let filtered_hyps = Evd.evar_filtered_hyps info in
  let env = Environ.reset_with_named_context filtered_hyps env in
  let Evd.Evar_defined bo = Evd.evar_body info in
  let ty = Retyping.get_type_of env sigma bo in
  let ctx = EC.named_context_of_val filtered_hyps in
  let ctx = ctx |> CList.filter (fun x ->
    not(CList.mem_f Id.equal (Declaration.get_id x) section)) in
  bo, ty, ctx, env
...
and in_elpi_solved_evar evar_body evar_concl coq_ctx ~calldepth ~depth state =
  let state, evar_body, gls_evar_body  = constr2lp coq_ctx ~calldepth ~depth state evar_body in
  let state, evar_concl, gls_evar_concl = constr2lp coq_ctx ~calldepth ~depth state evar_concl in
  debug Pp.(fun () -> str"in_elpi_solved_evar: moving Ctx down to" ++ int depth);
  let hyps = List.map (fun { E.hsrc = ctx_entry; hdepth = from } ->
    U.move ~from ~to_:depth ctx_entry) coq_ctx.hyps
    |> U.list_to_lp_list in
  state, hyps, evar_body, evar_concl, gls_evar_body @ gls_evar_concl
...
let rec postprocess_DeclareEvar calldepth k raw_ev elpi_evk state =
  ...
  let evar_concl, ctx, _ = info_of_undefined_evar ~env:global_env ~sigma ~section:(section_ids global_env) k in
...
let set_initial_sigma state sigma = S.update engine state (fun x -> assert(Evd.is_empty x.sigma); { x with sigma })
...
let sealed_goal2lp ~depth ~args ~in_elpi_tac_arg ~base state k =
  ...
  let evar_concl, goal_ctx, _goal_env =
    info_of_undefined_evar ~env ~sigma ~section:(section_ids env) k in
  let state, g, gls =
...
let sealed_solved_goal2lp ~depth state k =
  let calldepth = depth in
  let env = get_global_env state in
  let sigma = get_sigma state in
  let evar_body, evar_concl, goal_ctx, _goal_env =
    info_of_defined_evar ~env ~sigma ~section:(section_ids env) k in
  let state, g, gls =
    under_coq2elpi_ctx ~calldepth state goal_ctx
      ~mk_ctx_item:(fun _ t -> E.mkApp nablac (E.mkLam t) [])
      (fun coq_ctx ~depth state ->
            let tate, hyps, evar_body, evar_concl, gls = in_elpi_solved_evar evar_body evar_concl coq_ctx ~calldepth ~depth state in
          state, E.mkApp sealc (in_elpi_goal state ~args:[] ~hyps ~raw_ev:evar_body ~ty:evar_concl ~ev:evar_body) [], gls) in
  state, g, gls
...
let sealed_maybe_solved_goal2lp ~depth state k =
  let sigma = get_sigma state in
  if Evd.is_defined sigma k then sealed_solved_goal2lp ~depth state k
  else sealed_goal2lp ~depth state k 
...
let customtac2query sigma goals loc text ~depth:calldepth ~base state =
  ...
  | [goal] ->
    ...
    let evar_concl, goal_ctx, goal_env =
      info_of_undefined_evar ~env ~sigma ~section:(section_ids env) goal in
    let state, query, gls =
      under_coq2elpi_ctx ~calldepth state goal_ctx
      (fun coq_ctx ~depth state ->
  ...
  let state, assigned, changed, extra_gls =
    UVMap.fold (fun k _ _ elpi_solution (state, assigned, changed, extra) ->
      ...
      | Some t ->
       ...
       let _, ctx, _ = info_of_undefined_evar ~env:global_env ~sigma ~section:(section_ids global_env) k in
...
let sealed_goals2ev ~depth constraints state pp_ctx r =
  let syntactic_constraints = E.constraints constraints in
  let l, depth = skip_lams ~depth 0 (r) in
  if no_list_given l then None
  else
    ...
    let declared = CList.filter_map (fun x ->
      ...
      | Not_a_goal -> 
        err Pp.(str"Not a goal " ++ str(pp2string (P.term depth) x) ++ str " in " ++ cut () ++
          str(pp2string (API.Pp.constraints pp_ctx) constraints))) l in
    Some declared
...
let sealed_goals2coqgoals all_goals constraints state assignments pp_ctx =
   match API.Data.StrMap.find "NewGoals" assignments with
   | exception Not_found -> Evar.Set.elements all_goals , []
   | r ->
      match sealed_goals2ev ~depth:0 constraints state pp_ctx r with
      | None -> Evar.Set.elements all_goals, []
      | Some declared -> declared, Evar.Set.elements @@ Evar.Set.diff all_goals (List.fold_right Evar.Set.add declared Evar.Set.empty)
...
[%%else]
let poly_cumul_udecl_variance_of_options state options =
  let poly = options.poly in
  let udecl = options.universe_decl in
  state, poly, PolyFlags.cumulative poly, Option.default UState.default_univ_decl udecl, None
[%%endif]
...
let udecl_of_entry vars csts variances loose_udecl =
  ...
  let of_variances variances = 
    ...
    | Infer_variances -> 
      Array.map (fun x -> None) vars |> Array.to_list
  ...
  { univdecl_qualities = []; univdecl_extensible_qualities = loose_udecl;
    univdecl_elim_constraints = Sorts.ElimConstraints.empty;
    univdecl_instance = Array.to_list vars; univdecl_extensible_instance = loose_udecl;
    univdecl_variances = Option.map of_variances variances;
    univdecl_univ_constraints = csts; univdecl_extensible_constraints = loose_udecl }
...
let inductive_entry2lp ~depth coq_ctx constraints state ~loose_udecl e =
  ...
  let state = match mie.mind_entry_universes with
    ...
    | Polymorphic_ind_entry (cs, _variances) -> S.update engine state (fun e ->
        { e with sigma = merge_universe_context_set UState.univ_flexible e.sigma (snd (UVars.UContext.to_context_set cs)) }) (* ???? *) in
